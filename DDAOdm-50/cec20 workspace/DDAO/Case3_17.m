%% Problem Parameters
function [bestSolution,  bestFitness, iteration] = Case3_17(cec20so,dimension,maxIteration,i)
...
while(t < MaxIt) %t=1:MaxIt
    ...
    index = fitnessDistanceBalance(pop, pop(i).Cost );
        ...
        elseif (rem(t,2)==0)
            if(rand<0.9)
                Mnew.Phase = (pop(index).Phase-pop(index).Phase)+ bnew.Phase*rand;
            else
                Mnew.Phase = (pop(kk).Phase-pop(index).Phase)+ bnew.Phase*rand;
            end
        end
