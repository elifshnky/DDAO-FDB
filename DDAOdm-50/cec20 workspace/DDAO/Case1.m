%% Problem Parameters
function [bestSolution,  bestFitness, iteration] = Case1(cec20so,dimension,maxIteration,i)
...
while(t < MaxIt) %t=1:MaxIt
    ...
    index = fitnessDistanceBalance(pop, pop(i).Cost );
        ...
        if(rem(t,2)==1)
            if(rand<0.6)
                Mnew.Phase = (pop(index).Phase-pop(bb).Phase)+ bnew.Phase;          %EQUATION(1)
            else
                Mnew.Phase = (pop(kk).Phase-pop(bb).Phase)+ bnew.Phase;          %EQUATION
            end
        elseif (rem(t,2)==0)
            if(rand<0.6)
                Mnew.Phase = (pop(kk).Phase-pop(bb).Phase)+ bnew.Phase*rand;    %EQUATION(2)
            else
                Mnew.Phase = (pop(kk).Phase-pop(bb).Phase)+ bnew.Phase*rand;    %EQUATION
            end
        end
